...
import { ClipboardList, CheckCircle, Star, Calendar, Sparkles, Loader2 } from "lucide-react";
          ...
          <Card>
            ...
            {upcoming.length === 0 ? (
              <p className="text-sm text-[var(--color-text-muted)]">
                {t("common.noData")} —{" "}
                <Link href="/customer/book" className="text-[var(--color-primary)] underline">
                  Đặt dịch vụ ngay
                </Link>
              </p>
            ) : (
              <div className="space-y-3">
                {upcoming.map((o) => (
                  <Link
                    key={o._id}
                    href={`/customer/orders/${o._id}`}
                    className="flex items-center justify-between p-3 rounded-[var(--radius-md)] hover:bg-[var(--color-surface-hover)]"
                  >
                    <div>
